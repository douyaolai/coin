...
class PayController extends HomeController {
    ...
    public function rechargeByMan(){
    	...
    	if($member['status']!=1){
    		$arr['info']='请完成实名验证再进行充值,<br/><a href="/Home/ModifyMember/modify.html">点击这里现在去实名</a>';
    		$arr['status']=0;
    		$this->ajaxReturn($arr);
    	}
    ...
    public function bpay2(){
        ...
        $show       = $Page->show();// 分页显示输出
        $listlog=$pay->where($where)->order('pay_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        foreach ($list as $k=>$v){
    	...
    	// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
    	//$chongzhi = M("Tibi")->where($where)->order("id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
    	$this->assign('memberaccount',$memberaccount);// 账号
    	$this->assign('listlog',$listlog);// 赋值数据集
    	$this->assign('page',$show);// 赋值分页输出
    	$this->assign("list",$list);
    	$this->assign("currency",$currency);//货币信息
    	$this->display();
    ...
    //充币提交
    public function bpay2submit()
    {
    	if(IS_POST){
    		//参数检查
    		if(empty($_POST['lhty_account'])){
     			$this->error('请输入账户');
     		}
     		if(!isset($_POST['lhty_pwd'])){
     			$this->error('请输入交易密码');
     		}
    		if(empty($_POST['lhty_money'])){
     			$this->error('请输入充值金额');
     		}
			
			if($this->config['trancoin_enable']==0)
			{
				$this->error("系统配置，暂时无法转入",U("User/index"));
			}
    		
    		//参数读取
    		$lhty_account=I("lhty_account");
    		$lhty_pwd=I("lhty_pwd");
    		$lhty_money=I("lhty_money",0,'intval');
    		$lhty_type='cx';
    		
    		//接口访问
    		$apiResult= $this->request_lhty_api($lhty_account,$lhty_pwd,$lhty_money,$lhty_type);
    		//接口访问成功
    		if(strpos($apiResult, "Success")){
    			//通过验证 开始操作数据
    			//积分记录,更新信息  
            	//数据处理
	    		...
	    		$data['money'] = $lhty_money;
	    		$data['count'] = $lhty_money;
	    		$data['account'] = I('lhty_account');
	    		$data['status'] = 1;//状态成功
	     		$data['add_time']  = time();
	     		$data['type'] = 2;//自主充值类型  
	     		M()->startTrans();//开启事务	     		
	     		$r[] = M('Pay')->add($data);//添加记录	     		
	     		$r[] = M('Currency_user')->where(array('member_id'=>$data['member_id'],array('currency_id'=>$data['currency_id'])))->setInc('num',$data['money']);//改账户金额
	     		...
	     		if(!in_array(false,$r)){
	     			...
	     			M()->rollback();
	     			$this->error($data['currency_id']);
	     		}	
    		}
    		else { //接口访问失败			
    			$this->error($apiResult);
    		}   		
    ...
    //请求重消API
    private function request_lhty_api($username,$password,$amount,$type)
    {    	
    	$data = array("Comment" => "转入交易平台", "Amount" => $amount);
    	$data_string = json_encode($data);
    	$url='http://www.lianhe319.com/API/Account/ConsumeCoin';
    	//if($type=='cx')
    		//$url='http://www.lianhe319.com/API/Account/ConsumeChongXiao';
    	$ch = curl_init($url);
