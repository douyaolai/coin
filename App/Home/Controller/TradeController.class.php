...
class TradeController extends TradeFatherController {
    ...
    public function buy(){
        ...
        //启用价格限制
        if ($currency['price_down']>0 && $currency['price_up']>0){
            //跌停价格限制
            if($buyprice<$currency['price_down'])
			{
				$msg['status']=-9;
            	$msg['info']='交易价格超出了跌停价格限制' . $currency['price_down'];
            	$this->ajaxReturn($msg);
			}	
			//涨停价格限制
	        if ($buyprice>$currency['price_up']){
	            $msg['status']=-7;
	            $msg['info']='交易价格超出了涨停价格限制' . $currency['price_up'];
	            $this->ajaxReturn($msg);
	        }
            
        }else{//涨跌停
        	$tradendprice= $this->getradendprice($buycurrency_id);//收盘价		
			$priceup=$tradendprice * (1+$currency['price_updownpersent']/100);//涨停价
			$pricedown=$tradendprice * (1-$currency['price_updownpersent']/100);//跌停价
		
			//跌停价格限制
        	if ($tradendprice>0&&$buyprice<$pricedown){
            	$msg['status']=-9;
            	$msg['info']='交易价格超出了跌停价格限制' . $pricedown;
            	$this->ajaxReturn($msg);
        	}
			//涨停价格限制
	        if ($tradendprice>0&&$buyprice>$priceup){
	            $msg['status']=-7;
	            $msg['info']='交易价格超出了涨停价格限制' . $priceup;
	            $this->ajaxReturn($msg);
	        }
        }
 ...
 //      M()->query("UNLOCK TABLES");
	...
	public function sell(){
	   ...
	   if ($currency['is_lock']){
        ...
        if ($currency['price_down']>0 && $currency['price_up']>0){
            //跌停价格限制
            if($sellprice<$currency['price_down'])
			{
				$msg['status']=-9;
            	$msg['info']='交易价格超出了跌停价格限制' . $currency['price_down'];
            	$this->ajaxReturn($msg);
			...
			//涨停价格限制
	        if ($sellprice>$currency['price_up']){
	            $msg['status']=-7;
	            $msg['info']='交易价格超出了涨停价格限制' . $currency['price_up'];
	            $this->ajaxReturn($msg);
	        }
        	...
        	$tradendprice= $this->getradendprice($currency_id);//收盘价		
			$priceup=$tradendprice * (1+$currency['price_updownpersent']/100);//涨停价
			$pricedown=$tradendprice * (1-$currency['price_updownpersent']/100);//跌停价
		
			//跌停价格限制
        	if ($tradendprice>0&&$sellprice<$pricedown){
            	$msg['status']=-9;
            	$msg['info']='交易价格超出了跌停价格限制' . $pricedown;
            	$this->ajaxReturn($msg);
        	}
			//涨停价格限制
	        if ($tradendprice>0&&$sellprice>$priceup){
	            $msg['status']=-7;
	            $msg['info']='交易价格超出了涨停价格限制' . $priceup;
	            $this->ajaxReturn($msg);
	        }
