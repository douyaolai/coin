...
class OrdersController extends CommonController{
    ...
    public function index(){
        ...
        //如果设置了交易限额
        if($currency['price_up']>0 && $currency['price_down']>0)
		{
			$priceup=$currency['price_up'];
			$pricedown=$currency['price_down'];
		}else{
			//昨日收盘价 涨停价 跌停价
        	$tradendprice= $this->getradendprice($currency['currency_id']);//收盘价
        	$priceup=$tradendprice * (1+($currency['price_updownpersent']/100));//涨停价
			$pricedown=$tradendprice * (1-($currency['price_updownpersent']/100));//跌停价
		}       		
		
        
        $this->assign('tradendprice',$tradendprice);
        $this->assign('priceup',$priceup);
        $this->assign('pricedown',$pricedown);
