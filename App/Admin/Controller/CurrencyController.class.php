...
//     			M("Currency_user")->where("member_id='{$_SESSION['USER_KEY_ID']}' and currency_id='$cuid'")->setDec("num",$num);
     ...
     * 修改涨停跌停比例
     */
    public function savePricePersent(){
    	$currency_id = trim(I('id','','intval'));
    	if(empty($currency_id)){
    		$data['status'] = -1;
    		$data['info'] = '参数错误';
    	...
    	if(!is_numeric($_POST['price_updownpersent'])){
    		$data['status'] = -2;
    		$data['info'] = '填写必须是数字';
    		$this->ajaxReturn($data);
    	}
    	$r[] = M('Currency')->where("currency_id = {$currency_id}")->setField('price_updownpersent',$_POST['price_updownpersent']);    	
    	if(in_array($r, false)){
    		$data['status'] = -3;
    		$data['info'] = '服务器繁忙,请稍后重试';
    		$this->ajaxReturn($data);
