    ...
    //修正众筹表 计算剩余数量  修改状态
     ...
     protected  function  getCurrencyByCurrencyId($currency_id=0){
		if (empty($currency_id)) {
     	...
     	//获取交易币种信息
     	$list= M('Currency')->field("currency_id,currency_name,price_up,price_down,price_updownpersent,currency_buy_fee,currency_sell_fee,trade_currency_id,is_lock,rpc_url,rpc_pwd,rpc_user,port_number,currency_all_tibi")->where($where)->select();
    	if (!empty($currency_id)) {
    		return $list[0];  	
     	}else{
     		return $list;
     ...
     //获取上一个交易日的收盘价格
	protected function getradendprice($currency_id)
    {    	
        $start=strtotime(date('Y-m-d'));           
        
        $where['currency_id']=$currency_id;
        $where['type']='buy';
        $where['add_time']=array('lt',$start);    
        
        //收盘
        $order_s=M('Trade')->field('price')->where($where)->order('add_time desc')->find();            
        $order_s=!empty($order_s['price'])?floatval($order_s['price']):$order_k['price'];
        
        $item=!empty($order_s)?$order_s:0;
