                ...
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> 
                            ...
                            <td><?php echo ((isset($vo['price_updownpersent']) && ($vo['price_updownpersent'] !== ""))?($vo['price_updownpersent']):"0.000"); ?></td>                            
                            <td><?php echo (date("Y-m-d",$vo["add_time"])); ?></td>
                            <td>
                                <a class="link-update" href="<?php echo U('Trade/trade',array('currency_id'=>$vo['currency_id']));?>">查看交易记录</a></br>
                                <a class="link-limit" href="javascript:void(0);" onclick="limit(<?php echo ($vo['currency_id']); ?>,<?php echo ($vo['price_up']); ?>,<?php echo ($vo['price_down']); ?>)">设置交易限额</a><br>
                                <a class="link-limit" href="javascript:void(0);" onclick="limitpersent(<?php echo ($vo['currency_id']); ?>,<?php echo ($vo['price_updownpersent']); ?>)">设置涨跌停比例</a><br>
...
function limit(id,price_up,price_down){
	...
	layer.open({
	  ...
	  content: '<div style="margin-top:15px;text-align:center;"><p style="padding-bottom:15px;">限额优先级高于涨跌停，设置为零禁用限额</p><span>设置交易上限：</span><input type="text" name="price_up" value="'+price_up+'" id="price_up"/> ￥</div><div style="margin-top:15px;text-align:center;"><span>设置交易下限：</span><input type="text" value='+price_down+' name="price_down" id="price_down"/> ￥</div><div style="text-align:center;margin-top:15px;"><input type="button" value="确定" onclick="setlimit('+lid+');" style="padding:3px 10px;" /></div>'
	});

};
function limitpersent(id,price_updownpersent){
	var lid = id;
	var price_up = price_up;	
	layer.open({
	  title: '设置交易涨跌停比例',
	  ...
	  area: ['300px', '170px'], 
	  shadeClose: true, //开启遮罩关闭
	  content: '<div style="margin-top:15px;text-align:center;"><span>设置比例 </span><input type="text" name="price_updownpersent" value="'+price_updownpersent+'" id="price_updownpersent"/> % </div><div style="text-align:center;margin-top:15px;"><input type="button" value="确定" onclick="setlimitpersent('+lid+');" style="padding:3px 10px;" /></div>'
	});
...
function setlimitpersent(id){
	$.post("<?php echo U('Currency/savePricePersent');?>",{id:id,price_updownpersent:$("#price_updownpersent").val()},function(data){
		 if(data<0){
			layer.msg(data.info);
		 }else{
			layer.msg(data.info);
