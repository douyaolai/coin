    ...
    <div class="main-wrap">
       ...
       <div class="search-wrap">
            <div class="search-content">
				<div class="search-wrap">
					<div class="search-content">
						<form action="<?php echo U('Tongji/yue');?>" method="post">
							<table class="search-tab">
								<tr>
									<th width="120">选择分类:</th>
									<td>
										<select name="currency_id" id="">
                        				<?php if(is_array($currencylist)): $i = 0; $__LIST__ = $currencylist;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['currency_id']); ?>" <?php if(($cur["currency_id"]) == $vo["currency_id"]): ?>selected<?php endif; ?>><?php echo ($vo['currency_mark']); ?></option><?php endforeach; endif; else: echo "$empty" ;endif; ?>
										</select>
									</td>
                ...
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        <tr>
                            ...
                            <th>用户名(ID)</th>
                            <th>昵称</th>
                            <th>可用资产(<?php echo ($cur["currency_name"]); ?>)</th>
                            <th>冻结资产(<?php echo ($cur["currency_name"]); ?>)</th>
                            <th>总资产(<?php echo ($cur["currency_name"]); ?>)</th>
                        </tr>
                        <?php if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr>
                            <td><?php echo ($key); ?></td>
                     		<td><?php echo ($vo["member"]["member_id"]); ?></td>
                     		<td><?php echo ($vo["member"]["name"]); ?></td>
                     		<td><?php echo ($vo["num"]); ?></td>
                     		<td><?php echo ($vo["forzen_num"]); ?></td>
