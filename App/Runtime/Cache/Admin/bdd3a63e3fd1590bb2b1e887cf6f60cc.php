...
$(".sidebar-list li").children("a").on("click",function(){
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="<?php echo U('Index/index');?>">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">银行卡管理</span></div>
        </div>
        ...
        <div class="result-wrap">
            ...
            <div class="result-title">
                <div class="result-list">
                    <a href="<?php echo U('Websitebank/addBank');?>"><i class="icon-font"></i>新增银行卡</a>
                </div>
            </div>
            <div class="result-content">
                <table class="result-tab" width="100%">
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php echo ($vo['bank_adddress']); ?></td>
                            <td><?php if($vo['status'] == 1): ?>开通<?php endif; ?>
								<?php if($vo['status'] == 2): ?>关闭<?php endif; ?>
							
								</td>
                            <td>
                                <a class="link-update" href="<?php echo U('Websitebank/saveBank',array('bank_id'=>$vo['bank_id']));?>">修改|</a>
                               <!-- <a class="link-del" href="<?php echo U('Websitebank/delBank',array('bank_id'=>$vo['bank_id']));?>" onclick="return confirm('确认删除吗？')">删除</a>-->
								<a href="javascript:void(0)" class="link-del" onclick="cexiao(<?php echo ($vo["bank_id"]); ?>)">删除</a>
                            </td>
...
<script>
	function cexiao(_this){
	  ...
	  $.post('<?php echo U('Websitebank/delBank');?>',{bank_id:_this},function(data){
				   if(data['status'] == 1){
					   layer.msg(data['info']);
