...
<div id="main">
	<div class="main_box">
        ...
        <span style="font-weight:normal;color:#e55600;font-size:12px;padding-left: 15px;">推荐<?php echo ((isset($config["name"]) && ($config["name"] !== ""))?($config["name"]):'虚拟币'); ?>赚佣金，躺着也挣钱！</span></div>
	   ...
	   <!-- 账户总资产估算新改:-->
			...
			<tbody>
				<tr class="list_con1">
				    ...
				    <td class="cost assets03"><a href="<?php echo U('User/pay');?>">充值</a> &nbsp;&nbsp;<a href="<?php echo U('User/draw');?>">提现</a></td>
				    <td></td>
				</tr>
				<?php if(is_array($currency_user)): $i = 0; $__LIST__ = $currency_user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="list_con2">
					    ...
					    <td class="cost interest assets03">
					    	<?php if($config['trancoin_enable'] == 1): ?><a href="<?php echo U('Pay/bpay2',array('currency_id'=>$vo['currency_id']));?>">充币</a><?php endif; ?>
					    	
						<!--<a href="<?php echo U('Pay/bpay',array('currency_id'=>$vo['currency_id']));?>">充币</a>
                        &nbsp;&nbsp;<a href="<?php echo U('Pay/tcoin',array('currency_id'=>$vo['currency_id']));?>">提币</a>-->
						</td>
